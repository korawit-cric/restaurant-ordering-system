...
Orderly currently stores one plan-backed subscription per tenant. New restaurants receive a 30-day `starter` trial; the starter and standard plans limit active branches to one and two respectively. Subscription state is checked only when an owner creates another branch. There is no scheduler, renewal process, automatic billing, trial-expiry transition, grace policy, or subscription notification. Existing staff and public ordering continue when a trial expires or a subscription becomes `PAST_DUE` or `CANCELLED`.

The recommended design separates plan definition, billing state, effective access, and notification delivery. It evaluates access from persisted timestamps on every protected operation, uses a small durable scheduler for lifecycle milestones and provider reconciliation, writes lifecycle events and notification outbox rows transactionally, and preserves operational access for active orders during billing restriction.

See the [subscription lifecycle guide](SUBSCRIPTIONS.md) for the complete current behavior, operator limitations, proposed access modes, scheduler and notification architecture, manual and provider billing paths, model extensions, failure recovery, and implementation sequence.
